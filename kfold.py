...
def train(args):
    data = pd.read_parquet('./data/13w_recall_Top1000_Top20_sentence.parquet')
    data = data[data['type'] != '3w'].reset_index(drop=True)
    kfold = StratifiedKFold(n_splits=5)
    for fold, (train_idx, valid_idx) in enumerate(kfold.split(data, data['answer'])):
        trn = data.loc[train_idx].reset_index(drop=True)
        val = data.loc[valid_idx].reset_index(drop=True)
        trn['id'] = list(range(len(trn)))
        val['id'] = list(range(len(val)))
        val = val.sample(2000)
        val = val.reset_index(drop=True)
        trn['prompt'] = trn['context'].apply(lambda x : ' '.join(x)[:1750]) + ' #### ' + trn['prompt']
            ...
            with tqdm(train_dataloader) as pBar:
                ...
                for batch in pBar:
                    ...
                    if step % 200 == 0:
                        ...
                        if score > best_score:
                            torch.save(model.state_dict(), f'./save/kfold/fold{fold}_best.bin')
                            best_score = score
                        torch.save(model.state_dict(), f'./save/kfold/fold_{fold}_{step}.bin')
